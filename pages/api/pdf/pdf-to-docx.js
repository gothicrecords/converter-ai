  ...
  try {
    ...
    if (!apiKey) {
      return res.status(501).json({
        error: 'PDF->DOCX richiede CONVERTAPI_SECRET',
        hint: 'Aggiungi la variabile di ambiente CONVERTAPI_SECRET per ConvertAPI',
      });
    }

    const urlBase = `https://v2.convertapi.com/convert/pdf/to/docx?Secret=${encodeURIComponent(apiKey)}`;
    ...
    for (const f of list) {
      ...
      results.push({ name: f.originalFilename?.replace(/\.pdf$/i, '.docx') || 'converted.docx', url: file.Url, type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
    }
