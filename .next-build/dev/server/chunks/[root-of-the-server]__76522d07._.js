...
async function extractText(filePath, mimeType, originalFilename) {
    ...
    if (mimeType === 'application/pdf' || ext === '.pdf') {
        // Estrazione testo da PDF usando pdf-parse
        try {
            console.log('[PDF Extract] Inizio estrazione PDF:', originalFilename);
            // Dynamic require compatibile con Next.js API routes
            const pdfParse = (await __turbopack_context__.A("[externals]/pdf-parse [external] (pdf-parse, esm_import, async loader)")).default;
            console.log('[PDF Extract] pdf-parse caricato, tipo:', typeof pdfParse);
            ...
            if (!text || text.length < 50) {
                console.log('[PDF Extract] Testo insufficiente, provo OCR automatico...');
                // PDF probabilmente scansionato - usa OCR
                try {
                    const { data: { text: ocrText } } = await __TURBOPACK__imported__module__$5b$externals$5d2f$tesseract$2e$js__$5b$external$5d$__$28$tesseract$2e$js$2c$__cjs$29$__["default"].recognize(filePath, 'ita+eng', {
                        logger: (m)=>{
                ...
                throw new Error('Il PDF non contiene testo estraibile. Potrebbe essere composto solo da immagini non leggibili.');
            }
            console.log('[PDF Extract] Estrazione completata con successo');
            return text;
        } catch (error) {
            console.error('[PDF Extract] Errore estrazione PDF:', error.message);
            // Se l'errore non è già il nostro messaggio personalizzato, lancia errore generico
            if (error.message.includes('non contiene testo')) {
                throw error;
            }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__76522d07._.js.map