...
export async function generateImageWithDALLE(prompt, options = {}) {
  ...
  try {
    ...
    const style = options.style || 'vivid'; // 'vivid' or 'natural'
    const n = options.n || 1; // Number of images (1 for DALL-E 3)
    ...
    if (!imageResponse.ok) {
      throw new Error('Errore nel download dell\'immagine generata');
    }
    ...
    if (error.status === 401) {
      ...
      throw new Error(`Errore nella generazione: ${error.message || 'Prompt non valido o contenuto non consentito'}`);
    }
