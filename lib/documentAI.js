...
export async function extractTextFromDocument(buffer, mimeType, filename, filepath = null) {
  try {
    ...
    if (mimeType === 'application/pdf' || filename.toLowerCase().endsWith('.pdf')) {
      // Usa solo pdfjs-dist (pdf-parse ha problemi con Turbopack/Next.js)
      try {
        ...
        // Configura worker PRIMA di usare getDocument (importante!)
        // In Node.js, dobbiamo disabilitare il worker o usare un worker fittizio
        if (pdfjsLib.GlobalWorkerOptions) {
          // Usa un worker fittizio per Node.js
