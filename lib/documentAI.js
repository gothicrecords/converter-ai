  ...
  try {
    ...
    if (mimeType === 'application/pdf' || filename.toLowerCase().endsWith('.pdf')) {
      ...
      try {
        ...
        try {
          // Import corretto di pdf-parse - può essere default export o named export
          const pdfParseModule = await import('pdf-parse');
          // Prova diversi modi di accesso alla funzione
          let pdfParse = pdfParseModule.default;
          if (!pdfParse || typeof pdfParse !== 'function') {
            pdfParse = pdfParseModule;
          }
          if (!pdfParse || typeof pdfParse !== 'function') {
            // Se è un oggetto, prova a cercare la funzione
            pdfParse = pdfParseModule.pdfParse || pdfParseModule.parse;
          }
          
          // Verifica che sia una funzione
          if (!pdfParse || typeof pdfParse !== 'function') {
            console.error('pdf-parse module structure:', Object.keys(pdfParseModule));
            throw new Error(`pdf-parse non è una funzione valida. Tipo: ${typeof pdfParse}, Keys: ${Object.keys(pdfParseModule || {}).join(', ')}`);
          }
          ...
          // Fallback: prova a usare pdfjs-dist con percorso corretto
          console.log('pdf-parse fallito, provo con pdfjs-dist:', pdfParseError.message);
          
          try {
            // Prova diversi percorsi per pdfjs-dist
            let pdfjsLib;
            let getDocument;
            
            // Prova prima il percorso standard
            try {
              pdfjsLib = await import('pdfjs-dist');
              getDocument = pdfjsLib.getDocument || (pdfjsLib.default && pdfjsLib.default.getDocument);
            } catch (e1) {
              ...
              try {
                ...
                try {
                  ...
                  throw new Error(`Impossibile importare pdfjs-dist: ${e3.message}`);
                }
              }
            }
            ...
            // Configura worker per Node.js - necessario per pdfjs-dist
            if (pdfjsLib.GlobalWorkerOptions) {
              // In Node.js, disabilita il worker o usa un worker fittizio
              // Per estrazione testo, non serve il worker
              try {
                // Prova a impostare un worker fittizio
                pdfjsLib.GlobalWorkerOptions.workerSrc = 'data:application/javascript,';
              } catch (e) {
                // Se fallisce, prova a disabilitare completamente
                pdfjsLib.GlobalWorkerOptions.workerSrc = null;
              }
            }
            
            // Converti Buffer in Uint8Array per pdfjs-dist
            const uint8Array = new Uint8Array(buffer);
            
            // Usa la versione Node.js-friendly
            const loadingTask = getDocument({
              data: uint8Array, // Usa Uint8Array invece di Buffer
              useSystemFonts: true,
              verbosity: 0, // Riduce i log
              // Disabilita worker per Node.js
            ...
            // Se anche pdfjs-dist fallisce, rilancia l'errore originale di pdf-parse
            console.error('pdfjs-dist fallito:', pdfjsError);
            throw new Error(`Errore estrazione PDF: pdf-parse fallito (${pdfParseError.message}), pdfjs-dist non disponibile (${pdfjsError.message})`);
          }
        }
      } catch (pdfError) {
